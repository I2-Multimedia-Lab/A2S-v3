...
def tran_param(config):
    ...
    rot = (np.random.randint(*[-10,10]) // 1) * 1
...
def transform(image, is_mask=True, config=None):
    ...
    if is_mask:
        ...
        #temp = rotate(img=temp, angle=rot, interpolation=torchvision.transforms.InterpolationMode.NEAREST, expand=False, fill=0)

    else:
        temp = F.upsample(image, size=(input_size, input_size), mode='bilinear', align_corners=True)
        #temp = rotate(img=temp, angle=rot, interpolation=torchvision.transforms.InterpolationMode.BILINEAR, expand=False, fill=0)
